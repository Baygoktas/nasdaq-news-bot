...
function passes(item) {
  ...
  const matched = escapedTerms.find(term =>
    new RegExp(`(^|[^a-z0-9])${term}([^a-z0-9]|$)`, "i").test(title)
  );

  if (!matched) {
    console.log("FİLTRE DIŞI:", item.title);
    return false;
  }

  console.log("FİLTREDEN GEÇTİ:", matched, "|", item.title);
  return true;
}
...
function looksTurkish(text) {
  return /[çğıöşüÇĞİÖŞÜ]/.test(text) ||
    /\b(ve|ile|için|olan|bir|şirket|hisse|piyasa|sonuçları|açıklama|karar)\b/i.test(text);
}

function looksEnglish(text) {
  return /\b(the|and|for|with|from|into|after|before|shares|stock|company|approved|announces|reports|results|revenue|earnings|merger|acquisition)\b/i.test(text);
}
...
async function translate(text) {
  ...
  if (looksTurkish(input) && !looksEnglish(input)) {
    return input;
  }
  ...
  try {
    ...
    if (
      translated &&
      translated.toLowerCase() !== input.toLowerCase() &&
      !looksEnglish(translated)
    ) {
      return translated;
    }
  } catch (e) {
    console.log("Google çeviri hatası:", e.message);
  }
  ...
  try {
    ...
    console.log("MyMemory çeviri hatası:", e.message);
  }

  // İngilizce kaldıysa haberi göndermiyoruz.
  return "";
}
...
async function run(env) {
  ...
  for (const item of filtered) {
    ...
    // Türkçe çeviri başarısızsa İngilizce başlık gönderme.
    if (!title) {
      translationFailed++;
      continue;
    }

    // Özet mesajın içine eklenmez.
    // Telegram link önizlemesi / Anlık Görünüm tarafında gösterilir.
    const msg =
  ...
  return {ok:true,total:unique.length,filtered:filtered.length,sent,duplicate,translationFailed};
}
